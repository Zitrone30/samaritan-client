...
public class SamaritanFabricClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        KeyBinding openLoginScreen = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                ...
                GLFW.GLFW_KEY_K,
                KeyBinding.Category.create(Identifier.of("samaritan", "general"))
        ));
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildRootCommand(String rootLiteral) {
        return ClientCommandManager.literal(rootLiteral)
                                ...
                                .then(ClientCommandManager.argument("host", StringArgumentType.string())
                                        .then(ClientCommandManager.argument("port", IntegerArgumentType.integer(1, 65535))
                                                .executes(ctx -> {
                                                    ...
                                                    ctx.getSource().sendFeedback(Text.literal("Samaritan server set to " + protocolLabel(useTls) + "://" + host + ":" + port));
                                                    return 1;
                                                })
                                                .then(ClientCommandManager.argument("protocol", StringArgumentType.word())
                                                        .executes(ctx -> {
                                                            String host = StringArgumentType.getString(ctx, "host");
                                                            int port = IntegerArgumentType.getInteger(ctx, "port");
                                                            ...
                                                            if (useTls == null) {
                                                                ctx.getSource().sendError(Text.literal("Protocol must be http or https"));
                                                                return 0;
                                                            }
                        ...
                        .then(ClientCommandManager.literal("passwd")
                                .then(ClientCommandManager.argument("currentPassword", StringArgumentType.string())
                                        .then(ClientCommandManager.argument("newPassword", StringArgumentType.string())
                                                .executes(ctx -> {
                                                    MinecraftClient client = MinecraftClient.getInstance();
    ...
    private static int sendHelp(CommandContext<FabricClientCommandSource> ctx) {
        ...
        ctx.getSource().sendFeedback(Text.literal("/samaritan server <host> <port> [http|https]"));
        ...
        ctx.getSource().sendFeedback(Text.literal("/samaritan passwd <currentPassword> <newPassword>"));
        return 1;
    }
